...
app.get(
  ...
  passport.authenticate("google", {
    successRedirect: "https://animex-qdrqb.ondigitalocean.app/",
    failureRedirect: "https://animex-qdrqb.ondigitalocean.app/",
  })
);
...
app.get("/get-library/:libraryId", isUserAuthenticated, async (req, res) => {
  ...
  await Library.find({ user: req.session.passport.user }).then(async (data) => {
    ...
    const ids = response?.join();
    const getLib = await axios.get(
      `https://kitsu.io/api/edge/anime?filter%5Bid%5D=${ids}&page%5Blimit%5D=20`
    );
    res.json(getLib.data);
  });
});
