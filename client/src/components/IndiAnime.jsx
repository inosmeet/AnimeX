  ...
  const YoutubeThumbnail = ({ item }) => {
    return (
      <div className="yt-thumbnail relative  col-start-5 row-start-4 h-[70px] w-full ml-5 mt-3">
        <a
          href={
            "https://www.youtube.com/watch?v=" + item.attributes.youtubeVideoId
          }
          ...
          <table className="info-table ml-2 border-separate">
            <tbody>
              ...
              <tr>
                ...
                <td className="font-medium">
                  {item.attributes.episodeLength === null
                    ? "-"
                    : item.attributes.episodeLength + " minutes each"}
                </td>
              </tr>
            </tbody>
          </table>
    ...
    return (
      <div className="info-description relative col-start-3 row-start-3 col-span-2 ">
        {item.attributes.description.length < 300 ? (
          <p className="whitespace-pre-line">{item.attributes.description}</p>
        ) : isClicked ? (
        ...
        {include
          .map((link, ind) => {
            return (
              <div key={ind} className="inline mt-3 col-span-2">
                <li className="inline">
                  <Link
        ...
        <div className="h-[30px]">
          <Link
            to="/Most Popular Anime"
            className="text-[#464646] no-underline hover:text-[#464646] float-left"
          >
            {"❤ Rank #" +
          ...
          <Link
            to="/Highest Rated Anime"
            className="text-[#464646] no-underline hover:text-[#464646] float-right"
          >
            {"⭐ Rank #" +
      ...
      <>
        <div className="cover-img absolute col-span-6 w-full ">
          {item.attributes.coverImage === null ? (
            <img
              src={defaultImage}
              className="h-[450px] w-[inherit] object-cover absolute"
            />
          ) : (
            <img
  ...
  const AnimeTitle = ({ item }) => {
    return (
      <div className="info-title relative block col-start-3 row-start-2 col-span-full ">
        <span
          className="inline font-bold opacity-95 leading-none text-[60px] text-[#fff]"
          style={{ textTransform: "capitalize" }}
        >
          {item.attributes.slug}
        </span>
        <span className="text-[30px] text-[white] opacity-75 font-bold ml-[2%]">
          {item.attributes.startDate.slice(0, 4)}
        </span>
        <span className="text-[#1abc9c] block font-bold mt-[2%] ">
          {item.attributes.averageRating + "% Community Approval"}
        </span>
      </div>
    );
  };
