...
const AnimeDetails = ({item}) => {
    return(<div className="anime-details ml-5 relative col-start-5 row-span-1 row-start-3 w-full bg-white h-full rounded-md">
    <div className="mt-3">
...
const Description = ({item, include}) => {
  ...
  {include.map((link, ind) => { 
    ...
    <Link to="/Most Popular Anime" className="text-[#464646] no-underline hover:text-[#464646] float-left">{"❤ Rank #" + item.attributes.popularityRank + " (Most Popular Anime)"}</Link>
    <Link to="/Highest Rated Anime" className="text-[#464646] no-underline hover:text-[#464646] float-right">{"⭐ Rank #" + item.attributes.ratingRank + " (Highest Rated Anime)"}</Link>
    </div>
    <hr />
    </div>)
}
...
const AnimeTitle = ({item}) => {
    return(<div className="info-title relative block col-start-3 row-start-2 col-span-full ">
